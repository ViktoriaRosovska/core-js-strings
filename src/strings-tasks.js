...
function removeFirstOccurrences(str, value) {
  const newSubstr = str.indexOf(value);
  if (newSubstr !== -1) {
    return (
      str.substring(0, newSubstr) + str.substring(newSubstr + value.length)
    );
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const newSubstr = str.lastIndexOf(value);
  if (newSubstr !== -1) {
    return (
      str.substring(0, newSubstr) + str.substring(newSubstr + value.length)
    );
...
function findLongestWord(sentence) {
  ...
  let longWordIdx = 0;
  let maxLength = 0;
  sentenceArray.forEach((el, idx) => {
    if (el.length > maxLength) {
      longWordIdx = idx;
      maxLength = el.length;
    }
  });
  return sentenceArray[longWordIdx];
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const newArr = arr.map((el) => el.split('').reverse().join('')).join(' ');
  return newArr;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (idx >= 0) {
      const nc = alphabet.charAt((idx + 13) % 26);
      ch = ch.toUpperCase() === ch ? nc.toUpperCase() : nc;
    }
    st += ch;
  }
  return st;
...
function getCardId(value) {
  ...
  return cardArray.findIndex((el) => el === value);
}
